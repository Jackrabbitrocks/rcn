...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="single-portfolio sg-portfolio-layout9">
		...
		<div class="single-portfolio-main row">
			...
			<div class="single-portfolio-boxright col-xs-12 col-sm-5 col-md-4 col-lg-4">
				<div class="single-portfolio-about">
					<ul>
						<?php if(!empty($opt_meta_options['portfolio_client'])) { ?><li><span><?php echo esc_html__('Client: ', 'enormous'); ?></span><?php echo esc_attr($opt_meta_options['portfolio_client']); ?></li><?php } ?>
						<?php if(!empty($opt_meta_options['portfolio_location'])) { ?><li><span><?php echo esc_html__('Location: ', 'enormous'); ?></span><?php if(!empty($opt_meta_options['portfolio_location'])) { echo esc_attr($opt_meta_options['portfolio_location']); } ?></li><?php } ?>
						<li><span><?php echo esc_html__('Date: ', 'enormous'); ?></span><?php the_date(); ?></li>
						<li><span><?php echo esc_html__('Category: ', 'enormous'); ?></span><?php echo get_the_term_list( get_the_ID(), 'portfolio_categories' ); ?></li>
					</ul>
				</div>
				<div class="single-portfolio-share">
					<ul>
						<li><span class="ft-psb"><?php echo esc_html__('Share This Project: ', 'enormous'); ?></span></li>
						<?php enormous_cms_get_socials_share(); ?>
					</ul>
				</div>
			</div>
		</div>
		...
		<?php if(!empty($opt_meta_options['single_portfolio_gallery'])) { ?>
			<div class="single-portfolio-image sg-gallery sg-portfolio-masonry row">
				<?php foreach ($image_ids as $image_id): ?>
		            <?php
			            $attachment_image = wp_get_attachment_image_src($image_id, 'full', false);
			            if($attachment_image[0] != ''):?>
			            <div class="single-portfolio-image-item cms-images-zoom col-xs-12 col-sm-4 col-md-4 col-lg-4">
		...
		<?php } else { ?>
			<div class="single-portfolio-image">
				<?php enormous_post_thumbnail(); ?>
			</div>
		<?php } ?>

		<div class="single-portfolio-pagination clearfix">
			<div class="pt-line"></div>
			<?php enormous_post_nav_portfolio(); ?>
		</div>
