...
<div class="cms-grid-wraper cms-grid-demo <?php echo esc_attr($atts['template']);?>" id="<?php echo esc_attr($atts['html_id']);?>">
    ...
    <div class="row cms-grid <?php echo esc_attr($atts['grid_class']);?>">
        ...
        while($posts->have_posts()){
            ...
            $demo_custom_url = !empty($opt_meta_options['demo_custom_url']) ? $opt_meta_options['demo_custom_url']: '';
            $demo_url = !empty($opt_meta_options['demo_url']) ? $opt_meta_options['demo_url']: '';
            $home_url = home_url().'/?page_id='.$demo_url;
            ...
            <div class="cms-grid-item <?php echo esc_attr($atts['item_class']);?>" data-groups='[<?php echo implode(',', $groups);?>]'>
                <div class="cms-grid-item-inner rda_fadeInUp">
                    <a class="cms-grid-feature" href="<?php if(!empty($demo_custom_url)) { echo esc_url($demo_custom_url); } else { echo esc_url($home_url); } ?>" target="_blank" title="<?php the_title();?>">
                        <?php 
                    ...
                    <div class="cms-grid-title">
                        <h3><a href="<?php echo esc_url($home_url); ?>" target="_blank" title="<?php the_title();?>"><?php the_title();?></a></h3>
                    </div>
                </div>
            </div>
            <?php
