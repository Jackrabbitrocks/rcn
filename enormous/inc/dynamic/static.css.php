...
class CMSSuperHeroes_StaticCss
{
    ...
    public function css_render()
    {
        ...
        if(!empty($opt_theme_options['logo_max_height']['height']))
            echo '#cshero-header-logo .main_logo img{max-height:'.esc_attr($opt_theme_options['logo_max_height']['height']).';}';
        ...
        if(isset($opt_theme_options['menu_sticky']) && $opt_theme_options['menu_sticky']){  
            ...
            if(!empty($opt_theme_options['sticky_background_color']['rgba']))
                echo 'header #cshero-header.sticky-desktop.header-fixed{background-color:'.esc_attr($opt_theme_options['sticky_background_color']['rgba']).'}';
        }
        
        if(!empty($opt_theme_options['background_overlay_portfolio']['rgba'])) {
            echo '.single-portfolio #page-title.page-title .background-overlay {
                background-color:'.esc_attr($opt_theme_options['background_overlay_portfolio']['rgba']).'
            }';
        }
        return ob_get_clean();
    }
